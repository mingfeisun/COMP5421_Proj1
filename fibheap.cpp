...
{
pixel* temp_px = (pixel*)malloc(sizeof(pixel));
FibHeapNode Temp(temp_px);
int Result;
...
FibHeapNode* Temp = NULL;
	...
	Temp = Temp->Right;
     ...
     std::cout << "Done Printing.  Hit a key.\n";
         std::cin >> ch;
     }
}
...
// child during the consolidation).
     ...
     do {
//std::cout << "Top of Consolidate's loop\n";
//Print(w);
...
// may collide with another large tree on the root list.
        ...
        while (A[d] != NULL)
        {
		 ...
		 _Exchange(x, y);
             ...
             A[d] = NULL;
             d++;
//std::cout << "After a round of Linking\n";
//Print(x);
	}
	A[d] = x;
